...
mod tests {
    ...
    use super::*;
    use crate::{qr::encoder::QrEncoder, utils::test_utils::temp_dir};
    ...
    fn test_decode_command_empty_content() {
        let temp = temp_dir();
        let image_path = temp.path().join("empty.png");
    ...
    fn test_decode_command_unicode() {
        let temp = temp_dir();
        let image_path = temp.path().join("unicode.png");
    ...
    fn test_decode_command_long_text() {
        let temp = temp_dir();
        let image_path = temp.path().join("long.png");
    ...
    fn test_decode_command_special_chars() {
        let temp = temp_dir();
        let image_path = temp.path().join("special.png");
    ...
    fn test_decode_command_invalid_image() {
        let temp = temp_dir();
        let invalid_path = temp.path().join("not_an_image.txt");
    ...
    fn test_decode_command_different_qr_sizes() {
        ...
        for size in [100, 200, 400] {
            let image_path = temp.path().join(format!("size_{}.png", size));
    ...
    fn test_decode_command_different_error_correction() {
        ...
        for level in ["L", "M", "Q", "H"] {
            let image_path = temp.path().join(format!("ec_{}.png", level));
    ...
    fn test_decode_command_url_content() {
        let temp = temp_dir();
        let image_path = temp.path().join("url_content.png");
    ...
    fn test_decode_command_with_spaces_in_content() {
        let temp = temp_dir();
        let image_path = temp.path().join("spaces.png");
    ...
    fn test_decode_command_multiline_content() {
        let temp = temp_dir();
        let image_path = temp.path().join("multiline.png");
